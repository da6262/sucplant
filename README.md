...
| 분류 | 클래스 |
|------|--------|
| 버튼 | `.btn-primary` `.btn-secondary` `.btn-danger` `.btn-info` `.btn-warn` `.btn-neutral` `.btn-purple` `.btn-orange` `.btn-xs` `.btn-icon` `.btn-icon-edit` `.btn-icon-delete` |
| 배지 | `.badge` + `.badge-success` `.badge-warning` `.badge-danger` `.badge-info` `.badge-purple` `.badge-neutral` `.badge-sky` `.badge-orange` |
...
| **텍스트 컬러** | `.text-brand` `.text-brand-hover` `.text-info` `.text-danger` `.text-warn` `.text-heading` `.text-body` `.text-secondary` `.text-muted` `.text-amount` `.text-null-soft` |
| **배경 서피스** | `.bg-card` `.bg-section` `.bg-page` `.bg-info` `.bg-danger` `.bg-warn` `.bg-success` |
| **배경 액센트** | `.bg-info-accent` `.bg-success-accent` `.bg-warn-accent` `.bg-danger-accent` `.bg-purple-accent` `.bg-orange-accent` `.bg-sky-accent` |
| **폰트 크기** | `.text-3xs` `.text-2xs` `.text-xs` `.text-sm` `.text-base` `.text-lg` `.text-xl` |
...
| 버전 | 내용 |
|------|------|
| v3.4.93 | feat: 브라우저 confirm() 전면 교체 — utils/ui.js에 Promise 기반 showConfirm() 추가, 전체 11개 파일 29곳의 confirm() → 디자인 시스템 모달로 교체 (카테고리·대기자·주문·상품·고객·환경설정·SMS 발송 등) |
...
| v3.4.76 | feat: 고객 등록 화면 실용성 개선 — 사용자 피드백 "친화적이지 않음/실용적이지 못함" 반영. ①주소 input 클릭만으로 우편번호 검색 자동 오픈 (이전: Enter 키 강제, 어색함). readonly + cursor:pointer 로 시각 신호. handleAddressInput 빈 값 호출도 허용. ②전화번호 입력 중에는 중복 메시지 자동 숨김 (oninput 시 hidden), blur 시점에만 검증 → 빨간 글씨 거슬림 제거. ③Enter 키로 다음 필드 이동 (이름→전화→주소). 상세주소에서 Enter = 저장 (이미 Ctrl+Enter 지원, 추가). ④"추가 정보" 토글 더 명확하게 — 점선 박스 + "보통 비워두셔도 됩니다" 안내 텍스트로 신규 등록 시 부담 줄임 |
...
| v3.4.56 | fix: 고객관리 모바일 반응형 — 보조 버튼 4개(자동태그·등급관리·가져오기·내보내기) ⋮ 토글로 접기, 필터바 등급 버튼 flex-wrap 분리 |
...
| v3.4.39 | feat: 콜백대기 기능 — 고객관리 탭에 "콜백대기" 서브탭 추가. 고객 타임라인에서 "콜백요청" 타입으로 등록 → 콜백대기 탭에서 목록 확인 + 완료 처리. farm_customer_logs callback log_type 추가, metadata.done 플래그로 상태 관리 |
...
| v3.4.10 | feat: 문자 붙여넣기 — 기존/신규 고객 자동 처리. 기존 고객이면 파란 뱃지로 표시+customer_id 자동 연결, 신규 고객이면 주황 뱃지 안내 후 적용 시 farm_customers에 자동 등록. 미리보기 버튼 제거(붙여넣기 시 자동 인식). 전화번호 공백 구분자 지원 |
| v3.4.9 | feat: 문자 붙여넣기 → 주문 자동 입력 — 고객이 보낸 문자·카톡을 텍스트 영역에 붙여넣으면 이름·전화·주소를 자동 파싱하여 주문 등록 폼에 채움. 기존 고객 전화번호 매칭·라벨형/줄별 나열 양식 모두 지원. CLAUDE.md에 배포·충돌해결 규칙 추가 |
| v3.4.7 | fix: 고객관리 테이블 고객명 세로 표시 버그 수정 — 고객명·전화번호 컬럼 min-width 미지정으로 글자가 한 자씩 세로 나열되던 문제, th에 min-w 추가 + td에 whitespace-nowrap 적용 |
| v3.4.6 | fix: 사이드바 버전 배지 미표시 수정 — config.js에서 window.APP_VERSION 할당 누락으로 배지가 항상 "—"로 표시되던 버그 수정 |
...
| v3.3.141 | feat: 상품관리 테이블에 바코드 컬럼 추가 — PRODUCT_COLUMNS(product-management.js)·PRODUCT_TABLE_COLUMNS(productUI.js) 양쪽에 barcode 컬럼 삽입(사이즈 오른쪽), 상품 등록·수정 폼 "추가 정보" 섹션에 바코드 입력 필드 추가, formData 수집·fillProductForm·waitForFormFields 연동. DB 마이그레이션 SQL: add-product-barcode.sql |
| v3.3.138 | fix: 로그인 trim + 대시보드 새로고침 버튼 수정 — admin-auth.js 이메일·비밀번호 .trim() 추가(태블릿 자동완성 공백 처리), dashboard.js 새로고침 버튼 dashboardDataManager→dashboardComponent 교정 |

| v3.3.137 | feat(고객관리 Phase C): **자동 등급 승격 로그화**. ①`js/customer-management.js#updateCustomerGrade` — 업데이트 전 기존 등급 조회 → 변동 시에만 `farm_customer_logs` 에 `grade_change` 기록(`metadata: {old, new, old_label, new_label, reason:'auto_period', period, amount}`). 환경설정에 정의된 등급명(`grades[].name`)으로 라벨링 — 등급명 하드코딩 없음. 반환값에 `__changed` 플래그 추가. ②`features/customers/customerData.js#updateCustomer` — 수동 등급 변경 감지(`prevGrade !== incomingGrade`) → `reason:'manual'` 로 동일 로그. ③환경설정 "전체 고객 등급 재계산" 버튼 — `successCount`/`failCount` 외에 `changedCount` 누적, 완료 알림에 "등급 변경 N명 (나머지 유지)" 표시. ④`renderTimelineMetadata` — `grade_change` 메타를 `old_label → new_label · reason(한글) · period · amount원` 형태로 개선, 구 데이터(`old/new` code 만) 도 폴백. ⑤CLAUDE.md "고객 등급" 섹션 환경설정 기반임을 명시(하드코딩 금지·승격 경로 문서화). 후속 Phase D(RFM 이탈위험 자동 태그) / E(세그먼트 일괄 SMS) / F(엑셀 import/export) |
| v3.3.136 | feat: 로그인 모달 비밀번호 눈 아이콘(표시/숨기기) 토글 추가 |
...
| v3.3.40 | feat: 환경설정 `orderStatuses` ↔ 주문 등록 폼 드롭다운 + 주문관리 탭 상단 상태 필터 양방향 연결 — 기존에는 두 UI 가 모두 HTML 하드코딩(각 10개 옵션/11개 버튼)이라 환경설정 탭에서 상태를 추가/삭제해도 반영 안 되는 완전 dead setting 이었음. **변경 사항 (5단계)**: ①`features/orders/orderFormMinimalLayout.js#order-status` 의 하드코딩 `<option>` 10개 제거 (다른 세션이 되돌린 경우도 `populateOrderStatusSelectFromSettings()` 가 `innerHTML` 교체로 덮어씀). ②`features/orders/orderForm.js` 에 `populateOrderStatusSelectFromSettings(preservedValue?)` 신설 + `initOrderForm` 에서 호출 + `window.*` export. **orphan fallback**: 편집 모드에서 기존 DB 값이 환경설정에서 삭제된 상태면 `"XX (삭제됨)"` 라벨로 option 에 자동 추가해 값 보존. ③`components/order-management/order-management.html` 상태 탭 11개 하드코딩 중 **특수 탭 2개(`#status-work_todo`·`#status-all`) 만 정적 유지**, 개별 상태 탭은 `#status-tab-dynamic-slot` 에 동적 주입. ④`features/orders/orderData.js` 에 `renderStatusTabs()` 메서드 신설 — `settings.orderStatuses` 순회해 `<button class="status-tab-btn dynamic">` 생성 + `#status-tab-nav` 에 이벤트 위임(`dataset.delegatedStatusClick` guard) 로 클릭 핸들러 1회 바인딩. `loadOrders` 초입에서 idempotent 호출해 카운트 업데이트 전 탭 DOM 보장. ⑤`updateFilterCountsFromRpc` 의 `statusKeys` 하드코딩 12개 제거 → `document.querySelectorAll('.tab-count[id^="count-"]')` 로 현재 DOM 의 모든 count 엘리먼트를 동적 대상화. ⑥`features/orders/orderUI.js#fillOrderForm` 에서 편집 시 `populateOrderStatusSelectFromSettings(desiredStatus)` 를 먼저 호출해 orphan 케이스 대비. ⑦`features/settings/settingsUI.js` 의 "상태 추가" 버튼 성공 콜백에서 `orderDataManager.renderStatusTabs()` 호출로 주문관리 탭 즉시 반영. **범위 외**: 배송관리 상태 전이 검증(`shippingManager.js` 등 하드코딩), 기본 상태 삭제 방지 가드는 후속(v3.3.41+). 매치 로직(`matchOrderStatusFilter`) 은 변경 불필요 — 정확 문자열 매칭이라 새 상태 자동 지원 |
| v3.3.39 | docs: `README.md` **🧱 공통 컴포넌트 클래스** 테이블을 Phase 1B/C/D 결과로 확장 — 버튼 행에 `.btn-danger/.btn-info/.btn-warn/.btn-neutral/.btn-purple/.btn-orange/.btn-xs` 추가, 배지 행에 `.badge-sky/.badge-orange` 보완. 신규 행 4종: **텍스트 컬러**(11종)·**배경 서피스**(7종)·**배경 액센트**(7종)·**폰트 크기**(7단계). 이로써 README/CLAUDE.md 양쪽 모두 최신 유틸 테이블 동기화 완료 — 새 기여자·AI 편집기가 어디를 열든 같은 참조 소스 |
...
| v3.3.34 | refactor(Phase 1C 확대): 레거시 JS 진한 버튼 12건 시맨틱 교체 — Python 정규식 bulk. ①`js/shipping-management.js`(10건): 추적링크 복사(`btn-warn`)·배송 추가/확정/벌크 확정/템플릿 저장/내보내기 확정(`btn-info`×5)·엑셀 파일 선택(`btn-primary`)·템플릿 컬럼·행 추가·행 삭제(`btn-info btn-xs`/`btn-primary btn-xs`/`btn-danger btn-xs`)·수동 행 추가(`btn-info btn-xs`)·`disabled` 속성 유지. ②`js/order-management.js`(1건): 배송 설정 저장 → `btn-info`. ③`js/customer-management.js`(1건): 오류 시 새로고침 → `btn-danger btn-xs mt-3`. 보류 패턴: purple-600(배송 라벨 추가·규칙 저장·피킹 미리보기 2곳)·orange-600(로젠 엑셀 다운로드) — `.btn-purple/.btn-orange` 유틸 미정의, 추후 신설 시 일괄. classList 동적 bg-emerald-600/bg-gray-700 토글(`order-management.js:2762/2764`)도 구조적 리팩 필요로 보류 |
| v3.3.33 | refactor(Phase 1C 본격): 진한 Tailwind 버튼 조합 → 시맨틱 단일 클래스 5파일 16건 교체 — ①`features/orders/orderData.js`(6건): 일괄 작업 바 top + 활성 두 세트 각각 상태변경(`btn-info`)·일괄삭제(`btn-danger`)·선택해제(`btn-neutral`). ②`features/orders/orderSearch.js`(1건): 상품 검색 드롭다운 '완료' 버튼 `text-xs bg-blue-600 ...` → `btn-info btn-xs`(xs 사이즈 조합). ③`features/settings/settingsUI.js`(1건): 고객등급 편집 모달 '저장' → `btn-info`. ④`features/shipping/shippingManager.js`(6건): 헤더 새로고침·추적 조회·추적 목록 새로고침·추적링크 복사·라벨 인쇄·테스트 알림 → `btn-info/info w-full/neutral/warn/info/info`. ⑤`components/modals/picking-list-modal.html`(2건): 상품별 미리보기 → `btn-info`, 닫기 → `btn-neutral`. 크기 변화: 기존 `px-4 py-2 text-sm`(16/8/14px) → `.btn-*`(14/6/12px, 약간 축소) — 앱 전체 `.btn-primary` 스케일과 일관성 확보. v3.3.31 검증 통과 후 확대 적용 |
