...
### v3.2.14 — 2026-04-15 새 주문 등록 후 목록 미표시 버그 수정
- **주문 저장 후 렌더링 경쟁 조건 해결**: `setTimeout(1000ms)` → `window._pendingOrderStatus` 전역 플래그로 교체. 저장 직후 주문접수 탭으로 즉시 이동·표시

### v3.2.13 — 2026-04-15 고객관리 등급관리 버튼 먹통 수정
- **탭 재방문 시 이벤트 리스너 소실 버그**: `data-loaded` 조기 반환 조건 수정 — 탭 재방문 시에도 `initCustomerManagementSection()` 호출하여 이벤트 리스너 재연결

### v3.2.12 — 2026-04-15 커밋 시 버전 자동 증가
- **git pre-commit hook**: `hooks/pre-commit` — 커밋마다 `js/config.js` 패치 버전 자동 +1
- **setup-hooks.bat / setup-hooks.sh**: 개발 환경 초기 세팅 스크립트 추가
- **서버 버전 동기화**: `server.js` 시작 시 `README.md` 배지 버전 자동 업데이트

### v3.2.7 — 2026-04-15 표시 수 선택 기능 추가
- **4개 탭 하단바**: 고객·주문·상품·대기자 관리 화면 하단에 `표시 수` 드롭다운(10/20/50/전체) 추가
- **페이지 크기 적용**: `pageSize === 0` 이면 전체 표시, 그 외 상위 N건만 렌더링

### v3.2.6 — 2026-04-15 고객 목록 주문 추가 버튼 복구
- 고객 목록 테이블 행에서 주문 추가 버튼 복구

### v3.2.5 — 2026-04-15 고객 삭제 모달 개선
- **연관 주문 함께 삭제**: 고객 삭제 시 주문 목록 확인 모달 → 체크박스로 선택 삭제 기능 추가

### v3.2.4 — 2026-04-15 대기자 관리 및 다중 수정
- 대기자 관리 업데이트, 고객 삭제 시 연관 주문 함께 삭제 기능 추가
