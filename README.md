...
[![version](https://img.shields.io/badge/version-v3.3.44-brightgreen)](https://github.com/da6262/sucplant)
...
| 버전 | 내용 |
|------|------|
| v3.3.44 | fix: 상품 검색 장바구니 추가 블로킹 제거 + 재고 상태 색상 표시 — `addProductToCart` stock≤0 시 `alert`+`return` 차단 제거(품절 상품도 관리자 주문 등록 허용), 검색 결과 재고 표시 색상 시맨틱 변수 적용(품절=`var(--danger)` 빨강, 1~5=`var(--warn)` 주황, 6+=`var(--primary)` 초록); `farm_channels`에 "문자" 채널 추가(Supabase DB 직접 INSERT) |
