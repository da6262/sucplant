...
// 1. 앱 초기화 전에 로컬 모드 강제 활성화
window.FORCE_LOCAL_MODE = true;

// 인증 시스템은 모든 모드에서 활성화
console.log('🔐 인증 시스템은 강제 로컬 모드와 관계없이 활성화됩니다');

// 2. 모드 스위치 시스템과 통합 (로컬 모드로 강제 설정)
if (window.MODE_SWITCH) {
    window.MODE_SWITCH.currentMode = 'local';
    window.MODE_SWITCH.applyMode('local');
    console.log('✅ 모드 스위치 시스템과 통합됨 (로컬 모드)');
}

// 2. fetch 함수 오버라이드 활성화 (API 호출 완전 차단)
...
// 3. 앱 초기화 후 상태 강제 설정 (로컬 모드로 설정)
document.addEventListener('DOMContentLoaded', function() {
    setTimeout(() => {
        if (window.app) {
            console.log('🔄 앱을 로컬 모드로 강제 설정');
